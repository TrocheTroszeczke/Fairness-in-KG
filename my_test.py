...
if __name__ == "__main__":
    ...
    # load the model
    model = keras.models.load_model('./weight_bin/' + model_name_ + '.h5')
    ...
    if len(pos_triples_tmp) != len(neg_triples):
        raise ValueError('error when generating negative triples: len of pos != len of neg')

    pos_triples = pos_triples_tmp
    ...
    with open(f'fairness/{data_name}.txt', "w") as f:
        ...
        for i in tqdm(range(len(all_triples)), desc='relation corrupted ranking: evaluating'):
            ...
            subg_score = subgraph_triple_scoring(s, r, t, lower_bound, upper_bound_subg, one_hop_ind, id2relation,
                                                 model_2)
    ...
    ###Hit at 1#############################
    # generate the negative samples by randomly replace relation with all the other relation
    Hits_at_1 = 0
    Hits_at_3 = 0
    Hits_at_10 = 0
    MRR_raw = 0.
